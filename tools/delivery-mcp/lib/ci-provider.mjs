  ...
  async inspectCommit(sha, { repoRoot } = {}) {
    ...
    if (this.fixtures.has(sha)) {
      ...
      const res = {
        ...
        url: fix.url || `https://github.com/LoResuelvo/loresuelvo-admin-webapp/actions/runs/${fix.workflow?.id || 1001}`,
        retryable: Boolean(
...
export class GitHubActionsProvider extends CiProvider {
  constructor({ repo = "LoResuelvo/loresuelvo-admin-webapp", token = null } = {}) {
    super();
    this.repo = repo;
