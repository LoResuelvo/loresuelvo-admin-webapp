# AGENTS.md — Lo Resuelvo Admin Webapp
...
7. Ownership, commits, push, reportes y escalamiento se rigen por la granularidad declarada en `frontend-ai-development-workflow`. En `SCENARIO` y `SCENARIO_GROUP`, el desarrollador persistente puede completar, commitear y pushear únicamente el batch aprobado. La ventana de CI y los incidentes se evalúan automáticamente por el core y pre-push; el developer no realiza polling ni monitoreo manual por SHA tras cada push.
8. Cada commit debe ser coherente, compilable y testeable. Durante el ciclo interno RED/GREEN de TDD se utiliza exclusivamente `delivery_test` para comprobaciones focalizadas rápidas sin generar receipts de commit. En cada frontera atómica, con el cambio exacto staged, `delivery_prepare` selecciona y ejecuta el gate local (con soporte de jobs asíncronos recuperables mediante `delivery_job_wait` ante gates largos); solo `status: passed` habilita al agente a commitear y pushear a `main`. El flujo humano se define más abajo.
9. La ventana continua de CI (hasta cuatro commits totales en vuelo) y los incidentes activos se evalúan automáticamente por el core y los hooks; ante una falla se detienen nuevos pushes y se corrige la causa mediante el flujo de reparación auditable (`repair_ci` / Gate R). Queda prohibido cualquier bypass de CI (`DELIVERY_SKIP_CI_CHECK` es rechazado fail-closed). Un batch puede cerrarse localmente y dar paso al siguiente con CI pendiente dentro de esa ventana sin polling del developer.
10. La remoción del tag `@wip` se integra directamente en el commit funcional que deja el escenario GREEN. La US se cierra sobre HEAD sin commits artificiales (`delivery_verify_head`), con sus gates completos y CI verde (`delivery_finalize` con `waitForCi`).
...
```text
delivery_test({ mode: "affected" | "unit" | "scenario" | "diagnostic", ... }) # TDD; auto usa job si la ejecución puede ser larga
delivery_inspect({ intent, proposedCommitMessage, ... })       # previsualización opcional
delivery_prepare({ intent, proposedCommitMessage, ... })       # gate obligatorio sobre staged (modo sync/job)
delivery_job_wait({ jobId, timeoutMs })                        # espera acotada de jobs largos (Gate C/D/R, verify-head o CI)
delivery_job_cancel({ jobId, reason })                         # cancelación excepcional y auditable de un job recuperable
delivery_prepare({ intent: "repair_ci", repairsSha, ... })     # Gate R: reparación de un solo uso ante CI rojo
delivery_verify_head({ intent, scopeFiles, usId, mode: "job" }) # Gate D sobre HEAD; intent debe coincidir con close_batch/close_us
delivery_ci_inspect({ sha })                                   # diagnóstico puntual de CI ante fallos
delivery_finalize({ intent: "close_batch", usId, scopeFiles })  # cierre local; CI pendiente permitido
delivery_finalize({ intent: "close_us", usId, scopeFiles, waitForCi: true }) # cierre con Gate D + CI verde (soporta modo job)
```

El agente edita y usa Git normalmente, pero no ejecuta manualmente `make`, lint, typecheck, suites ni comandos crudos de CI como flujo habitual. Durante el ciclo RED/GREEN utiliza exclusivamente `delivery_test` para iteración rápida sin generar receipts de commit; su modo de ejecución predeterminado `auto` mantiene unitarios focalizados síncronos y deriva `scenario`, `affected` o diagnósticos largos a jobs recuperables. En la frontera atómica utiliza `delivery_prepare` sobre el snapshot staged. Ante gates largos (Gate C, D o R), verificación de HEAD o espera de CI, las herramientas admiten jobs recuperables: devuelven un `jobId` que se consulta mediante esperas acotadas con `delivery_job_wait`. `delivery_job_cancel` se reserva para cancelación explícita, no para reemplazar una espera normal. El servidor MCP devuelve resultados procesados compactos. Ante indisponibilidad o timeout de un job, se exige recuperación o escalación; queda prohibido el bypass con CLI o comandos directos.
...
Los comandos de `make` (`make test-e2e-managed`, etc.) y suites crudas son de uso interno para la ejecución de checks dentro de los gates o para diagnóstico focalizado puntual; no forman parte del flujo habitual de trabajo de los agentes.
...
- **Agentes autónomos**: MCP `delivery_prepare` es la entrada canónica obligatoria sobre el snapshot staged antes de cada commit. El guard anticipatorio disponible en el cliente deniega `git commit` si no existe un receipt válido y coincidente. Si el MCP requerido no está disponible, detenerse; la CLI neutral (`npm run delivery:*`) solo sustituye al MCP en un entorno aprobado explícitamente o para uso humano.
- **Humanos**: desarrollan, realizan stage y pueden commitear directamente ejecutando tests de forma manual o delegando la verificación a CI. En ausencia de receipt previo, el commit se registra en el ledger local como `not_run` y se permite tanto el commit como el push (salvo que se configure `DELIVERY_REQUIRE_EVIDENCE=1`). Para reparar un CI fallido sin Gate R deben registrar, después del último `git add`, `npm run delivery:context -- --intent repair_ci --repairs-sha <sha-fallido> [--us-id <id>]`; el contexto solo vale para ese parent, branch, árbol staged y mensaje compatible.
...
- **Ventana de CI sin polling del developer**: La ventana continua de hasta cuatro commits en vuelo y los incidentes activos se evalúan automáticamente por `delivery_prepare` y `pre-push`. El desarrollador no debe realizar polling ni monitorear periódicamente CI por SHA tras cada push.
- **Prohibición total de bypass de CI**: La variable `DELIVERY_SKIP_CI_CHECK` está prohibida y cualquier intento de utilizarla es rechazado inmediatamente de forma fail-closed (`DEPRECATED_CI_BYPASS_REJECTED`). No existe ningún bypass ambiental para eludir las comprobaciones de CI ni se permite `--no-verify`.
- **Flujo de reparación de CI (`repair_ci`)**: Ante una falla de CI, pre-push bloquea nuevos pushes ordinarios. Los agentes requieren Gate R, que reproduce los checks de CI asignados a agentes y emite un receipt. Los humanos pueden usar ese gate o registrar un contexto manual exacto y quedar `not_run`; no es un bypass y `DELIVERY_REQUIRE_EVIDENCE=1` lo bloquea. Ambas rutas exigen `repairsSha`, validan linaje y generan una autorización de push de un solo uso.
- **Superficie Docker reservada a humanos (HUMAN_ONLY)**: Modificaciones a `Dockerfile`, `.dockerignore`, `compose*.yml`, workflows de Docker o scripts de construcción de imágenes pertenecen exclusivamente a desarrolladores humanos. Los agentes se detienen con `HUMAN_ONLY_CHANGE` y escalan a `STOP_USER`. Un fallo observado en un job Docker puede usar `repair_ci` cuando su causa y el snapshot staged pertenecen únicamente a código no reservado.
- **Verificación sin commits artificiales (`delivery_verify_head`)**: Al finalizar una User Story o batch cuando HEAD ya contiene los cambios definitivos y no restan tags `@wip`, no se deben generar commits vacíos o artificiales. Se invoca con el mismo intent del cierre posterior: `close_batch` para un batch completo o `close_us` para la US. Gate D puede ejecutarse como job con `mode: "job"` y asocia la evidencia requerida a HEAD.
